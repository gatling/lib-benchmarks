...
public class GatlingBoonFastChopBenchmark {
	...
	private JsonFastParser newChopParser() {
		JsonFastParser jsonParser = new JsonFastParser(false, true, false, false);
	...
	public Object parseChars(ThreadState state) throws Exception {
		...
		Object json = newChopParser().parse(UnsafeUtil.getChars(new String(bytes, StandardCharsets.UTF_8)));
	...
	public Object parseStream(ThreadState state) throws Exception {
		...
		Object json = newChopParser().parse(stream, StandardCharsets.UTF_8);
		return BYTES_AND_JSONPATHS[i].path.query(json);
	}
}
